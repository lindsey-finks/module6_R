...
**Answer Q.1**
Uncertainty refers to the range of possible future outcomes due to the inability of being 100% sure of conditions at this point in time. 
...
**Answer Q.2**
Uncertainty can help natural resource managers transmit forecasts to the public in an efficient way. If a forecast has high uncertainty, that information can be relayed with the forecast to avoid giving false predictions.
...
**Answer Q.4**
  Lakes and inland water bodies can experience alterations chlorophyll-a concentrations or alterations in patterns of underwater ecosystems as a result of increasing water temperature.
...
**Answer Q.5**
It looks like there is a relationship between air temperature and water temperature in this lake. Generally, when air temperature rises water temperature rises.
...
**Answer Q.6**
  The gray lines on the plot represent ensemble members and they are potential outcomes of the forecast using different initial input conditions.
...
**Answer Q.7**
  The multiple linear regression model predicts water temperature by relating daily water temperature by water temperature lagged by one day as coefficients then relates air temperature.
...
**Answer Q.8**
  Past water temperature drives current water temperature while current air temperature has somewhat of an influence on current water temperature. The standard errors for both parameters are relatively low. If the standard error is closer to 1, there is less confidence in the fitted model parameter values.
...
**Answer Q.9** 
  With a high R2 of 0.9, the model performed well against the observations. 
...
**Answer Q.11** The deterministic forecast follows the trend of ensemble members in the NOAA GEFS forecast. The forecast for water temperature on Oct. 1 has high uncertainty amongst ensemble members, but the deterministic forcast looks to be the mean of ensemble members from the NOAA GEFS plot.
...
**Answer Q.12** The range of potential forecasts becomes larger and forecast uncertainty becomes larger.
...
**Answer Q.13** 
  The deterministic forecast is towards the lower end of temperatures when compared to the probabilistic forecast plot. The uncertainty in the probabilistic plot makes me think that the forecast for temperature throughout the week will be largely uncertain.
...
**Answer Q.14** To reduce parameter uncertainty, we could collect and use more data in the forecast. 
...
**Answer Q.15** If the forecast contains a lag or if the amount of time it takes for the initial condition to be updated it could affect the initial conditions uncertainty. 
...
**Answer Q.16** Parameter uncertainty seems to contribute the most to total forecast uncertainty.
...
**Answer Q.17** I think different water temperature forecast models would have similar quantities of uncertainty as the parameters that drive water temperature could be uncertain themselves.
...
**Answer Q.19** Water temperature is being predicted as a function of historic water temp., air temp., wind and shortwave radiation. This model includes more parameters than the previous model.
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!!!!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$wind_speed[4]
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$wind_speed * coeffs[4] + rnorm(30, mean = 0, sd = sigma)
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$wind_speed * coeffs[4]
...
**Answer Q.25** Parameter uncertainty still contributes the most to the total forecast uncertainty. 
